...
DB_MADE_BY = os.getenv("DB_MADE_BY")
print(f"\t\tMade By : {DB_MADE_BY}")
...
for i in range(1, num_records + 1):
    staff_data.append([i, fake.name(), round(random.uniform(30000, 80000), 2), random.choice(departments), DB_MADE_BY])

# Generate study room data
study_room_data = [['room_no', 'capacity', 'hourly_rate', 'made_by']]
for i in range(1, num_records + 1):
    study_room_data.append([i, random.randint(2, 10), round(random.uniform(15, 50), 2), DB_MADE_BY])
...
for i in range(1, num_records + 1):
    ...
    booking_data.append([i, random_date, random_time, discount, DB_MADE_BY])

# Generate payment data
payment_data = [['payment_id', 'payment_mode', 'amount', 'made_by']]
payment_modes = ['Credit Card', 'Debit Card', 'Paypal', 'UPI', 'Bank Transfer']
for i in range(1, num_records + 1):
    payment_data.append([i, random.choice(payment_modes), round(random.uniform(50, 500), 2), DB_MADE_BY])
...
for i in range(1, num_records + 1):
    student_data.append([i, fake.name(),fake.phone_number(), random.choice(universities), DB_MADE_BY])
...
for i in range(1, num_records + 1):
    coworking_space_data.append([random.choice(brands), f"Campus {i}", DB_MADE_BY])
